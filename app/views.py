...
def search_specializations(request):
    ...
    specializations = Specialization.objects.filter(status=1).filter(name__icontains=query)
...
def add_specialization_to_applicant(request, specialization_id):
    ...
    serializer = ApplicantSerializer(draft_applicant, many=False)

    return Response(serializer.data["specializations"])


@api_view(["GET"])
def get_specialization_image(request, specialization_id):
    if not Specialization.objects.filter(pk=specialization_id).exists():
        return Response(status=status.HTTP_404_NOT_FOUND)

    specialization = Specialization.objects.get(pk=specialization_id)
    response = requests.get(specialization.image.url.replace("localhost", "minio"))
